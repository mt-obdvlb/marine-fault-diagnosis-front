<template>
  <div :class="['area',screen.type]">
    <img v-show="screen.type==='ver'"
         class='floatBtn buttonEffect'
         src='/icon/more.svg'
         @click='panelMenuOn=!panelMenuOn'>
    <div v-show="screen.type==='hor'||panelMenuOn"
         class='panels'>
      <div v-for='p in panels'
           :class="['panel','buttonEffect',{'active':currentPanel.name===p.name}]"
           @click='currentPanel=p,panelMenuOn=false'>
        {{ p.name }}
      </div>
    </div>
...
const currentPanel = shallowRef(panels[0])
