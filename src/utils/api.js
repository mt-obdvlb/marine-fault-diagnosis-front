...
const api = {
    ...
    sessionRename(body, arg = {}, opt = {}) {
        return F.myFetchApi('/rename_session', { ...arg }, { method: 'PUT', json: body, ...opt });
    },
    ...
    qaGet(query = {}, arg = {}, opt = {}) {
        return F.myFetchApi(`/get_qa${buildQueryString(query)}`, { ...arg }, opt);
    },
    qaUpdate(body, arg = {}, opt = {}) {
        return F.myFetchApi('/update_qa', { ...arg }, { method: 'PUT', json: body, ...opt });
    },
    qaDelete(body, arg = {}, opt = {}) {
        return F.myFetchApi('/delete_qa', { ...arg }, { method: 'DELETE', json: body, ...opt });
    },
    ...
    referenceGet(questionId, arg = {}, opt = {}) {
        return F.myFetchApi(`/v1/reference_files?question_id=${encodeURIComponent(questionId)}`, { ...arg }, opt).then((r) => {
            ...
            return {
                ...
                data: {
                    doc_url: files[0]?.url || '',
                    knowledge_items: files.map((file) => ({ name: file?.name || '', category: file?.type || '' }))
                }
            };
        });
    },
    ...
    systemUpdateConfig(body, arg = {}, opt = {}) {
        return F.myFetchApi('/config/llm', { ...arg }, { method: 'PUT', json: body, ...opt });
    }
};
